  ...
  return (
    <div className='flex-1 p-[50px]'>
      <section className='w-full flex flex-col itmes-center justify-center gap-[20px] py-[50px]'>
        <h2 className='text-center text-[22px] font-semibold'>비밀번호 변경</h2>
        ...
        <div className='flex flex-col items-center'>
          ...
          <button
            className={`w-full h-[50px] flex items-center justify-center rounded-md bg-white shadow-sm ${(isEmailValid && isEmailVerifyNumberValid && isPasswordValid && isPasswordsValid && isTyped) ? 'hover:bg-blue-100' : ''}`}
            onClick={handlePassWordChangeClick}
            disabled={(!isEmailValid || !isEmailVerifyNumberValid || !isPasswordValid || !isPasswordsValid || !isTyped)}
          >
            <span className='font-semibold text-black'>비밀번호 변경</span>
          </button>
        </div>
      </section>
    </div>
